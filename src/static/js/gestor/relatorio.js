...
// VARIÁVEIS DA API
const baseURLColaborador = "http://127.0.0.1:5000/gestor/equipe"; 
const baseURLFeedback = "http://127.0.0.1:5000/colaborador/feedback"; 
...
        headers: { "Authorization": `Bearer ${token}` }
    ...
    if (!res.ok) {
            ...
            alert("Erro ao carregar lista de colaboradores. Verifique o servidor.");
            return [];
        }
...
function renderSearchResults(query) {
    // Fecha o dropdown se a pesquisa estiver vazia
    if (query.length < 2) {
    ...
    // Filtra colaboradores pelo nome (case insensitive)
...
function handleSelectColaborador(event) {
    ...
    // 3. Fecha e limpa os resultados
    searchResultsDiv.innerHTML = '';
    isSearchDropdownOpen = false;

    // 4. Habilita o botão de envio
